...
export default function Climate() {
    ...
    try {
        useEffect(() => {
            axios
                .get(`${process.env.REACT_APP_EN_API_URL}home-json-en/generateCategory7.json`)
                .then(({ data }) => {
                    if (data.length > 0) {
                        setList1(data[0]);
            ...
            <>
                <div className="job-area-news">
                    <div className="row">
                        <div className="col-12">
                            <div className="section-heading">
                                <h2><i className="fa-solid fa-chevron-right"></i><Link to='/english/climate' onClick={scrollTop}>Climate</Link></h2>
                            </div>
                        </div>
                    </div>
                        ...
                        List2.map((item, index) => {
                            return (
                                <div className="CommonLeadList" key={index}>
                                    <Link to={"/english/details/" + item.Slug + "/" + item.ContentID} onClick={scrollTop} >
                                        <div className="row">
                    ...
                    <div className="seeMore">
                        <Link className="btn btnMore" to="/english/climate" onClick={scrollTop}>More...</Link>
                    </div>
                </div>
            </>
        )
