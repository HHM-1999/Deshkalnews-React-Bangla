...
export default function ReligionAndLife() {
    const [List1, setList1] = useState([])
    const [List2, setList2] = useState([])
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_API_URL}home-json-bn/generateCategory25.json`)
            .then(({ data }) => {
                // if (data.length > 0) {
                setList1(data[0]);
        ...
        <>
            <div className="science-tech-area-news">
                <div className="row">
                    <div className="col-12">
                        <div className="section-heading">
                            <h2><i className="fa-solid fa-chevron-right"></i><Link to='/religion-and-life' onClick={scrollTop}>ধর্ম ও জীবন</Link></h2>
                        </div>
                    </div>
                </div>
                {List1 ?
                ...
                <div className="seeMore">
                    <Link className="btn btnMore" to='/religion-and-life' onClick={scrollTop}>আরও...</Link>
                </div>
            </div>
        </>
    )
