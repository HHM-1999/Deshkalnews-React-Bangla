...
export default function SportsMobile() {
    ...
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_API_URL}home-json-bn/generateCategory9.json`)
            .then(({ data }) => {
                if (data.length > 0) {
                    setList1(data[0]);
                    setList2(data[1])
                    setList3(data.slice(2, 4))
                    setTimeout(function () {
                        lazyloaded = false
        ...
        axios
            .get(`${process.env.REACT_APP_API_URL}category/sports`)
            .then(({ data }) => {
                if (data.category) {
    ...
    return (
        <>
            <div className="row">
                <div className="col-12">
                    <div className="section-heading d-flex align-items-end">
                        <h2><Link to="/sports" onClick={scrollTop}><i className="fa-solid fa-chevron-right"></i>খেলা</Link> </h2>
            ...
            <div className="seeMore">
                <Link className="btn btnMore" to="/sports" onClick={scrollTop}>আরও...</Link>
            </div>
